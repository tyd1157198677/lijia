...
<script>
import { listorder } from '@/api/middleground/order/commodity'
import edit from '@/views/order/commodity/edit'
import add from '@/views/order/commodity/add'
export default {
  name: 'arrange',
  components: {
    edit,
  ...
  methods: {
    ...
    handleExport() {
      ...
      if (this.ids.length != 0) {
        this.download(
          '/system/szm/goods/order/exportOrdersPart',
          {
            ...ids,
          },
    ...
    handleExportAll() {
      this.download(
        '/system/szm/goods/order/exportOrdersAll',
        {
          ...this.form,
        },
        `data_${new Date().getTime()}.csv`
