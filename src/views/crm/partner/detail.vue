  ...
  <div class="app-details">
    <el-form class="detatitle" ref="form" :model="form"  size="mini">
      <el-row>
        ...
        <el-col :span="8">
          <el-form-item label="提供服务项目数：">{{form.serviceTotalNum}}</el-form-item>
        </el-col>
        <el-col :span="12" >
          <el-form-item label="最近1次服务时间：">{{form.lastServiceTime}}</el-form-item>
        </el-col>
      </el-row>
    </el-form>
    <div class="app-details">
...
import {
  getpartnerInfo,
  getpartnerAttend,
} from '@/api/middleground/crm/partner'
...
export default {
  ...
  methods: {
    ...
    getList() {
      var that = this
      that.loading = true
      let userId = that.detailsrow.userId
      that.userId = userId
      getpartnerInfo(userId).then((response) => {
        that.form = response.data
        getpartnerAttend(that.queryParams, userId, that.itemNamerow).then(
          (response) => {
